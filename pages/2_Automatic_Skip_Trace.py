...
def read_existing_keys() -> set[str]:
    try:
        ...
        if "text/html" in str(response.headers.get("content-type", "")).lower():
            return set()
        existing = pd.read_csv(io.BytesIO(response.content))
        if "lead_key" not in existing.columns:
            return set()
        return {
            clean_text(value)
            for value in existing["lead_key"].tolist()
            if clean_text(value)
        }
    except Exception:
        return set()
...
existing_sheet_keys = read_existing_keys()
session_keys = set(st.session_state.get("skiptrace_sent_keys", []))
all_sent_keys = existing_sheet_keys | session_keys
unsent = queue[~queue["lead_key"].isin(all_sent_keys)].copy()

col1, col2, col3 = st.columns(3)
col1.metric("Waiting for skip trace", len(queue))
col2.metric("Already sent", len(queue) - len(unsent))
...
send_mode = st.radio(
    "Send mode",
    ["Safe test — first 1 unsent lead", "Send all unsent leads"],
    index=0,
)

if st.button("Run Automatic Skip Trace", type="primary", disabled=unsent.empty):
    batch = unsent.head(1).copy() if send_mode.startswith("Safe test") else unsent.copy()
    results: list[dict] = []
    ...
    st.session_state["skiptrace_sent_keys"] = sorted(session_keys)
