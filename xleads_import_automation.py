...
This module is intentionally separate from the lead-ranking engine so a bad CRM
credential or malformed export cannot break Seller Lead Command.
"""
...
PROPERTY_COLUMN_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "phone": ("Contact1Phone_1", "Contact1Phone1", "Phone", "phone"),
    "phone_2": ("Contact1Phone_2", "Contact1Phone2", "Phone 2", "phone_2"),
    "phone_type": ("Contact1Phone_1_Type", "Contact1Phone1Type", "Phone Type"),
    "phone_dnc": ("Contact1Phone_1_DNC", "Contact1Phone1DNC", "DNC"),
    "phone_litigator": ("Contact1Phone_1_Litigator", "Contact1Phone1Litigator", "Litigator"),
...
def series_from(df: pd.DataFrame, aliases: Iterable[str], default: str = "") -> pd.Series:
...
def truthy(value: Any) -> bool:
    return clean_text(value).lower() in {"1", "true", "yes", "y", "checked", "x", "dnc"}
...
def safe_slug(value: str) -> str:
    ...
    return slug[:80] or "xleads-import"
...
def _read_csv_bytes(data: bytes) -> pd.DataFrame:
    ...
    for options in attempts:
        try:
            return pd.read_csv(BytesIO(data), low_memory=False, **options)
...
def read_xleads_upload(filename: str, data: bytes) -> tuple[pd.DataFrame, str]:
    """Read a CSV directly or extract the most likely CSV from an XLeads ZIP."""
    lower_name = clean_text(filename).lower()
    ...
    try:
        ...
        raise ValueError("The uploaded ZIP is damaged or is not a valid ZIP file.") from exc


def _any_raw_dnc(df: pd.DataFrame) -> pd.Series:
    dnc_columns = [
        column for column in df.columns
        if any(token in normalize_header(column) for token in ("dnc", "donotcall", "litigator", "blacklist"))
    ]
    if not dnc_columns:
        return pd.Series([False] * len(df), index=df.index, dtype="bool")
    flags = pd.Series([False] * len(df), index=df.index, dtype="bool")
    for column in dnc_columns:
        flags = flags | df[column].map(truthy)
    return flags


def prepare_sync_dataframe(raw_df: pd.DataFrame, campaign_tag: str) -> pd.DataFrame:
    """Create a stable owner/property queue from the wide XLeads export."""
    if raw_df.empty:
        raise ValueError("The XLeads export has no rows.")

    out = pd.DataFrame(index=raw_df.index)
    for target, aliases in PROPERTY_COLUMN_ALIASES.items():
    ...
    out["phone_2"] = out["phone_2"].map(clean_phone)
    ...
    out["property_address"] = out.apply(
        lambda row: join_address(row["property_street"], row["property_city"], row["property_state"], row["property_zip"]), axis=1
    )
    out["mailing_address"] = out.apply(
        lambda row: join_address(row["mailing_street"], row["mailing_city"], row["mailing_state"], row["mailing_zip"]), axis=1
    )
    out["phone_dnc"] = out["phone_dnc"].map(truthy)
    out["phone_litigator"] = out["phone_litigator"].map(truthy)
    out["dnc_hold"] = out["phone_dnc"] | out["phone_litigator"] | _any_raw_dnc(raw_df)
    ...
    name_mail_key = (
        out["seller_name"].str.lower().str.replace(r"[^a-z0-9]", "", regex=True)
        + "|"
        + out["mailing_address"].str.lower().str.replace(r"[^a-z0-9]", "", regex=True)
    )
    ...
    property_counts = (
        out.loc[out["contact_key"].ne("") & out["property_key"].ne(""), ["contact_key", "property_key"]]
        .drop_duplicates()
        .groupby("contact_key")["property_key"]
        .nunique()
    )
    ...
    out["sync_action"] = "READY_TO_SYNC"
    out["sync_reason"] = "Valid owner/property record"
    out.loc[out["duplicate_owner_property"], ["sync_action", "sync_reason"]] = ["SUPPRESS_DUPLICATE", "Duplicate owner and property row"]
    out.loc[out["multi_property_review"] & ~out["duplicate_owner_property"], ["sync_action", "sync_reason"]] = [
        "MULTI_PROPERTY_REVIEW",
        "One contact is tied to multiple properties; hold to prevent overwriting a property field",
    ]
    no_match_key = out["phone"].eq("") & out["email"].eq("")
    out.loc[no_match_key, ["sync_action", "sync_reason"]] = ["NEEDS_CONTACT_MATCH", "No valid phone or email for CRM upsert"]
    no_property = out["property_address"].eq("")
    out.loc[no_property, ["sync_action", "sync_reason"]] = ["NEEDS_PROPERTY_DATA", "Property address is missing"]
    out.loc[out["dnc_hold"], ["sync_action", "sync_reason"]] = ["DNC_HOLD", "DNC, litigator, or blacklist flag found"]
...
def _build_tags(row: pd.Series) -> str:
    ...
    action = clean_text(row.get("sync_action"))
    action_tags = {
    ...
    if action in action_tags:
        tags.append(action_tags[action])
...
class HighLevelConfig:
    ...
    def configured(self) -> bool:
        required = (
            self.token,
            self.location_id,
            self.property_address_field,
            self.property_city_field,
            self.property_state_field,
            self.property_zip_field,
        )
        return all(clean_text(value) for value in required)
...
class HighLevelClient:
    """Minimal HighLevel v3 contact upsert client.

    The app only calls this after a human approves the dry run. It never starts a
    workflow or sends a text/call.
    """
    ...
    def upsert_contact(self, row: pd.Series) -> dict[str, Any]:
        response = requests.post(
            f"{self.config.api_base.rstrip('/')}/contacts/upsert",
            headers=self.headers,
            json=build_highlevel_payload(row, self.config),
...
def sync_safe_rows(queue: pd.DataFrame, client: HighLevelClient) -> pd.DataFrame:
    ...
    safe = queue[queue["safe_to_sync"]].copy()
    for _, row in safe.iterrows():
        try:
            ...
            results.append({
                ...
                "sync_result": "SUCCESS",
                "contact_id": clean_text(contact.get("id", "")) if isinstance(contact, dict) else "",
                "error": "",
            })
        except Exception as exc:
            results.append({
...
CRM_EXPORT_COLUMNS = [
    ...
    "property_street", "property_city", "property_state", "property_zip", "property_address",
    "owner_type", "avm", "wholesale_value", "mls_status", "mls_list_price",
    "mls_agent_name", "mls_agent_phone", "mls_agent_email", "crm_tags", "sync_action", "sync_reason",
]


def crm_export(queue: pd.DataFrame) -> pd.DataFrame:
    columns = [column for column in CRM_EXPORT_COLUMNS if column in queue.columns]
